...
export async function POST(request: NextRequest, { params }: Props) {
  ...
  const evenement = await prisma.evenement.findUnique({
    where: { id },
    select: { dateDebut: true, typeEvenement: true, titre: true, adversaire : true, equipeId : true },
  });
  ...
  if (maintenant.isBefore(limiteTempsEvenement))
    return NextResponse.json({
      message:
        "Vous devez attendre 3 heures après le début de l'événement pour afficher vos stats",
    }, {status : 400});
